...
当一个搜索请求到达要给节点，该节点就成了协调节点，然后会广播到索引每一个分片拷贝(主 + 副)，每个分片在本地构建一个优先队列，优先队列大小取决于分页参数，比如`from 90 size 10`的话就会创建100容量的优先队列。分片搜索和排序后会返回要给轻量级的结果列表到协调节点，协调节点整合后返回给客户端
